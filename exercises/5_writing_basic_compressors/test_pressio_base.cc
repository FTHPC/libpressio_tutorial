...
int main(int argc, char *argv[])
{
  ...
  struct pressio_compressor* compressor = pressio_get_compressor(library, "runlength");
  assert(compressor && "compressor must be registered");


  assert(pressio_compressor_major_version(compressor) == RUNLEGTH_MAJOR_VERSION);
  assert(pressio_compressor_minor_version(compressor) == RUNLEGTH_MINOR_VERSION);
  assert(pressio_compressor_patch_version(compressor) == RUNLEGTH_PATCH_VERSION);
  assert(strcmp(pressio_compressor_version(compressor), CONCAT(RUNLEGTH_MAJOR_VERSION, RUNLEGTH_MINOR_VERSION, RUNLEGTH_PATCH_VERSION) ) == 0);

  const char* prefix = pressio_compressor_get_prefix(compressor);
  assert(strcmp(prefix, "runlength") == 0);

  struct pressio_options* options = pressio_compressor_get_options(compressor);
  assert(options);
  assert_type_is(options, "runlength:value_to_encode", pressio_option_double_type);
  ...
  assert_type_is(documentation, "runlength:value_to_encode", pressio_option_charptr_type);
  ...
  pressio_options_set_double(options, "runlength:value_to_encode", 0.0);
  ...
  assert_data_same(input, decompressed);
